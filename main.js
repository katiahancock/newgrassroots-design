...
function getAllBills() {
  return fetch('https://openstates.org/api/v1/bills/?state=vt&apikey=2a939a8d-1448-4810-b036-79139a6a7f33&format=json&search_window=session')
    .then((response) => {
...
document.getElementById('submitButton').addEventListener('click', (event) => {
  getAllBills().then((currentSessionBills) => {
    ...
    for (let i = 0; i < results.length; i++) {
      if (results[i].bill_id.replace(/[\s]/, '') === searchTerms.replace(/[\W*_*]/gi, '') || results[i].title.replace(/[\s]/, '') === searchTerms.replace(/[\W*_*]/gi, '')) {
